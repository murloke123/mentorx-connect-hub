...
const MeusCursosPage = () => {
  ...
  // Determinar qual aba deve estar ativa baseado nos parâmetros da URL
  const getDefaultTab = () => {
    const tabParam = searchParams.get('tab');
    if (tabParam === 'meus-cursos') {
      return 'meus-cursos';
    }
    return 'cursos-adquiridos'; // Default original
  };

  const [activeTab, setActiveTab] = useState(getDefaultTab());

  // Limpar parâmetro da URL após definir a aba (para não persistir)
  useEffect(() => {
      ...
      <div className="flex-1 p-6">
        <div className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Gerencie seus cursos e acompanhe cursos adquiridos</p>
          </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="grid w-full grid-cols-2">
            ...
            <TabsTrigger value="cursos-adquiridos" className="flex items-center gap-2">
          ...
          <TabsContent value="cursos-adquiridos">
            <div>
              <h2 className="text-xl font-semibold mb-4">Cursos que Adquiri</h2>
              {loadingEnrolled ? (
