  ...
  return (
    <Card className={`bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 border border-gold/30 rounded-2xl backdrop-blur-xl shadow-lg hover:border-gold/50 transition-all duration-300 hover:shadow-gold/30 ${className}`}>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          {tooltipText && (
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent className="max-w-xs bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 border border-gold/30 text-white">
                  <p>{tooltipText}</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
        ...
        {icon && <div className="h-5 w-5 text-gold">{icon}</div>}
      </CardHeader>
      <CardContent>
        ...
        {trend && (
          <div className={`flex items-center text-xs mt-2 ${trend.positive ? 'text-green-400' : 'text-red-400'}`}>
